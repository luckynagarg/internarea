...
export default function VerifyOtpPage() {
  ...
  const [countDown, setCountDown] = useState<number>(300); // seconds
  ...
  const verify = async () => {
    ...
    try {
      ...
      setMessage(e?.response?.data?.error?.message || "OTP verification failed.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const resend = async () => {
    ...
    try {
      ...
      setMessage("If an account exists, we sent a new OTP.");
    } catch (e: any) {
      setMessage(e?.response?.data?.error?.message || "Resend failed.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-xl mx-auto px-4">
        <div className="bg-white rounded-2xl shadow p-6 md:p-8">
          ...
          <p className="text-gray-600 mt-2 text-sm">
            Enter the 6-digit OTP sent to your {method === "phone" ? "phone" : "email"}.
          </p>
          ...
          <div className="mt-4 text-sm text-gray-600">
            OTP expires in <span className="font-semibold">{Math.floor(countDown / 60)}:{String(countDown % 60).padStart(2, "0")}</span>
          </div>
